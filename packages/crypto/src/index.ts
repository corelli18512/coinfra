/**
 * @coinfra/crypto — hybrid end-to-end encryption primitives.
 *
 ...
 *
 * This is the Node.js implementation using the built-in `node:crypto` module.
 * A browser implementation using the Web Crypto API would share the same
 * interface. Knows nothing about any application: it moves opaque strings.
 */
...
export interface BlobPayload {
  /** base64(iv ‖ ciphertext ‖ tag) — the transport sees this as an opaque string */
  blob: string;
  /** Per-recipient RSA-OAEP encrypted AES key (base64), keyed by deviceId */
  keys: Record<string, string>;
}
 ...
 *
 * @param payload - The EncryptedPayload received over the wire
 ...
 * The blob is base64(iv ‖ ciphertext ‖ tag).
 */
export function decryptFromBlob(
 ...
 *
 * Only top-level keys are sorted; nested objects are stringified as-is. A flat
 * payload is assumed by design, so this is sufficient.
 */
